...
int main(int argc, char **argv)
{
	...
	log_size = filesize;
	log_addr = (uint64_t)mmap(NULL, log_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
	if(log_addr == (uint64_t)MAP_FAILED){
		perror("mmap");
		exit(EXIT_FAILURE);
	}

	madvise((void *)log_addr, log_size, MADV_RANDOM);
  ...
  if(err == 0){ // success call
    ...
    err = ioctl(fd, FAKE_BLK_IOC_FILL_RANGE, &frang); // will read from device
    //err = ioctl(fd, FAKE_BLK_IOC_ZERO_RANGE, &frang); // will **NOT** read from device
    if(err){
      printf("ioctl(FAKE_BLK_IOC_FILL_RANGE) failed! Program exiting...\n");
      exit(EXIT_FAILURE);
    }
  }
#endif
